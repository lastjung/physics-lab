...
import { RollerCoasterSpringCanvasRenderer } from '../render/rollerCoasterSpringCanvasRenderer';
import { RollerCoasterSpring } from '../simulations/rollerCoasterSpring';
import { mountRollerCoasterSpringControls } from '../ui/rollerCoasterSpringControls';
...
export const rollerCoasterSpringPlugin: SimulationPlugin = {
  id: 'roller-coaster-spring',
  create(context: PluginContext): ActivePlugin {
    const defaults = RollerCoasterSpring.getDefaultParams();
    const model = new RollerCoasterSpring({
      ...defaults,
      ...
      springK: readValue(context.initialValues, 'springK', 'sk') ?? readValue(context.presetValues, 'springK') ?? defaults.springK,
      springDamping:
        readValue(context.initialValues, 'springDamping', 'sd') ?? readValue(context.presetValues, 'springDamping') ?? defaults.springDamping,
      springRestX:
        readValue(context.initialValues, 'springRestX', 'srx') ?? readValue(context.presetValues, 'springRestX') ?? defaults.springRestX,
    ...
    const renderer = new RollerCoasterSpringCanvasRenderer(context.canvas, model);
    let lastImpactAt = 0;

    const redraw = () => {
      const [xRaw, vxRaw] = model.getState();
      if (Math.abs(vxRaw) > 5.2) {
        model.setState([xRaw, Math.sign(vxRaw) * 5.2]);
      }
      ...
      context.onSpringMotion?.(k.speed, k.springDx);
      context.onStats(
        `t=${model.getTime().toFixed(2)}s | x=${k.x.toFixed(2)} | springDx=${k.springDx.toFixed(2)}`,
    ...
    mountRollerCoasterSpringControls(context.menuRoot, model, redraw);
